...
The consequence was invisible until a host stayed up long enough. Under prolonged uptime, a
reopen resumed the session and then logged "already
...
def same_boot(monkeypatch):
    ...
    def _install(engine, *, pane="%16", live_recv=False):
        entry = {"pane": pane, "briefed_boot": BOOT, "engine": engine}
        monkeypatch.setattr(daemon, "read_registry", lambda: {"4108": entry})
...
def test_the_guard_lets_a_claude_briefing_through_on_a_host_that_has_not_rebooted(same_boot):
    """The regression itself: prolonged uptime must not silence the cause.
    ...
    assert daemon._briefing_still_ours("%16", "4108", "claude", "delivery") is True, (
...
def test_a_codex_pane_briefed_this_boot_is_not_briefed_again(same_boot):
    ...
    assert daemon._briefing_still_ours("%16", "4108", "codex", "delivery") is False
...
def test_a_codex_pane_from_an_earlier_boot_is_briefed(same_boot):
    ...
    assert daemon._briefing_still_ours("%16", "4108", "codex", "delivery") is True
...
def test_a_claude_pane_with_a_live_listener_is_left_alone(same_boot):
    ...
    assert daemon._briefing_still_ours("%16", "4108", "claude", "delivery") is False
...
def test_the_ownership_guard_still_rejects_a_pane_the_topic_has_left(same_boot, engine):
    """#133 r2, and it must survive this change: one chain typed topic A's briefing into %15
    ...
    assert daemon._briefing_still_ours("%15", "4108", engine, "delivery") is False
