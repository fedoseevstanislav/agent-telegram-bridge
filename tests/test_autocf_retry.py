...
def _run_worker(token="t"):
    ...
    daemon._cf_mark_unfinished("33", token)
    daemon._carry_forward_worker({}, 33, "%0", "/x/cf.md", "/x/cf.md.done", token, "sess")
...
def test_starting_a_carry_forward_opens_the_record(monkeypatch):
    ...
    monkeypatch.setattr(daemon.threading, "Thread",
                        lambda **kw: type("T", (), {
                            "start": lambda _s: at_start.append(daemon._cf_unfinished_since("33"))
                        })())
    monkeypatch.setattr(daemon, "_pending_cf", {})
    assert daemon.handle_carry_forward({}, 33, "/carryforward", {"name": "s"}, "%0") is True
...
def test_a_run_whose_record_cannot_be_written_does_not_start(monkeypatch):
    ...
    monkeypatch.setattr(daemon, "_pending_cf", {})
    assert daemon.handle_carry_forward({}, 33, "/carryforward", {"name": "s"}, "%0") is False
    assert started == []                                  # no worker
    assert not daemon.carry_forward_active(33)            # and no pending flow left behind
...
def test_a_worker_thread_that_will_not_start_leaves_nothing_behind(monkeypatch):
    ...
    monkeypatch.setattr(daemon, "_pending_cf", {})
    assert daemon.handle_carry_forward({}, 33, "/carryforward", {"name": "s"}, "%0") is False
    assert daemon._cf_unfinished_since("33") is None
    assert not daemon.carry_forward_active(33)
...
def test_settle_timeout_leaves_the_record_open(monkeypatch):
    ...
    _run_worker()
    assert daemon._cf_unfinished_since("33") is not None
...
def test_an_exempt_topic_is_not_promised_a_retry_it_will_never_get(monkeypatch):
    ...
    replies = []
    _worker_harness(monkeypatch, replies=replies, wait_idle="timeout")
    monkeypatch.setattr(daemon, "load_autocf_exempt", lambda: {"33"})
    _run_worker()
    [notice] = [r for r in replies if "stayed mid-turn" in r]
    assert "/cf" in notice
...
def test_a_run_that_compacts_does_not_close_another_runs_record(monkeypatch):
    ...
    replies = []
    daemon._cf_mark_unfinished("33", "older-run")
    _worker_harness(monkeypatch, replies=replies)
...
def test_the_record_closes_before_the_resume_not_after_the_run(monkeypatch):
    ...
    def watching_inject(tid, token, pane, text, settle=0.5, release_after=False):
        if text == daemon.CF_RESUME_PROMPT.format(path="/x/cf.md"):
            at_inject.append(daemon._cf_unfinished_since("33"))
...
def test_a_recent_run_keeps_the_topic_armed_even_if_the_flag_was_lost(monkeypatch):
    ...
    _aged("33", 60)                      # a run started a minute ago; the daemon then died
    armed = {}                           # ...and the flag never made it to disk
    assert daemon._process_autocf({}, "33", {}, "%0", 95, "claude", set(), armed) is False
    assert rec["cf"] == []
    assert armed["33"] is True           # the record is holding the topic armed
...
def test_record_writes_and_deletes_happen_under_the_lock(monkeypatch):
    ...
    daemon._cf_mark_unfinished("33", "t")
    daemon._cf_clear_unfinished("33", "t")
...
def test_a_record_too_malformed_to_attribute_is_removed(monkeypatch):
    ...
    with open(daemon._cf_unfinished_path("33"), "w") as f:
        f.write("1756000000")            # epoch only: no token to match against
    daemon._cf_clear_unfinished("33", "some-run")
    assert daemon._cf_unfinished_since("33") is None
...
def test_the_record_is_read_back_from_a_file(monkeypatch):
    ...
    assert daemon._cf_unfinished_since("33") == 1756000000.0
...
def test_only_the_run_that_opened_a_record_can_close_it():
    daemon._cf_mark_unfinished("33", "run-B")
    daemon._cf_clear_unfinished("33", "run-A")      # an older worker, finally reaching its end
    assert daemon._cf_unfinished_since("33") is not None
    daemon._cf_clear_unfinished("33", "run-B")
    assert daemon._cf_unfinished_since("33") is None
...
def test_the_consumer_closes_a_record_without_a_token():
    ...
    daemon._cf_mark_unfinished("33", "run-B")
    daemon._cf_clear_unfinished("33")
    assert daemon._cf_unfinished_since("33") is None
...
def test_re_arms_once_the_cooldown_has_passed(monkeypatch):
    ...
    _aged("33", 901)
    armed = {"33": True}
    fired = daemon._process_autocf({}, "33", {}, "%0", 95, "claude", set(), armed)
    assert fired is True
    assert rec["cf"] == [33]
    assert armed["33"] is True                       # armed again by the new run
    assert daemon._cf_unfinished_since("33") is None    # record consumed, not re-used
...
def test_does_not_re_arm_inside_the_cooldown(monkeypatch):
    ...
    assert armed["33"] is True
    assert daemon._cf_unfinished_since("33") is not None  # still pending, not discarded
...
def test_many_ticks_inside_the_cooldown_fire_nothing(monkeypatch):
    ...
    armed = {"33": True}
    for _ in range(20):
        daemon._process_autocf({}, "33", {}, "%0", 95, "claude", set(), armed)
    assert rec["cf"] == []
...
def test_does_not_re_arm_while_a_carry_forward_is_running(monkeypatch):
    ...
    assert rec["cf"] == []
    assert armed["33"] is True
    assert daemon._cf_unfinished_since("33") is not None  # kept for a tick that can use it
...
def test_an_exempt_topic_is_untouched_by_the_retry(monkeypatch):
    ...
    _aged("33", 901)
    armed = {"33": True}
    fired = daemon._process_autocf({}, "33", {}, "%0", 95, "claude", {"33"}, armed)
    assert fired is False
...
def test_a_compacted_run_stays_armed_until_the_context_drops(monkeypatch):
    ...
    assert daemon._process_autocf({}, "33", {}, "%0", 95, "claude", set(), armed) is False
    assert armed["33"] is True
    assert daemon._process_autocf({}, "33", {}, "%0", 10, "claude", set(), armed) is False
    assert armed["33"] is False
    assert rec["cf"] == []
...
def test_an_older_worker_cannot_close_a_newer_runs_record(monkeypatch):
    ...
    replies = []
    _worker_harness(monkeypatch, replies=replies)
    daemon._cf_mark_unfinished("33", "B")            # the newer run, still owed a retry
    daemon._carry_forward_worker({}, 33, "%0", "/x/cf.md", "/x/cf.md.done", "A", "sess")
    assert daemon._cf_unfinished_since("33") is not None
...
def test_a_halted_worker_that_never_compacted_closes_nothing(monkeypatch):
    ...
    replies = []
    _worker_harness(monkeypatch, replies=replies, wait_idle="timeout")
    daemon._cf_mark_unfinished("33", "B")
    daemon._carry_forward_worker({}, 33, "%0", "/x/cf.md", "/x/cf.md.done", "A", "sess")
    assert daemon._cf_unfinished_since("33") is not None
...
def test_exempting_a_topic_drops_its_pending_retry(monkeypatch):
    ...
    _aged("33", 10)
    daemon._process_autocf({}, "33", {}, "%0", 95, "claude", {"33"}, {"33": True})
    assert daemon._cf_unfinished_since("33") is None
...
def test_a_closed_topic_drops_its_pending_retry(monkeypatch):
    ...
    _aged("33", 10)
    _run_one_warning_poll(monkeypatch, {"33": {"pane": "%0", "closed": True}})
    assert daemon._cf_unfinished_since("33") is None
...
def test_a_codex_topic_drops_its_pending_retry(monkeypatch):
    ...
    _aged("33", 10)
    _run_one_warning_poll(monkeypatch, {"33": {"pane": "%0"}}, engine="codex")
    assert daemon._cf_unfinished_since("33") is None
...
def test_a_record_survives_a_daemon_that_died_mid_run(monkeypatch):
    ...
    assert daemon._cf_unfinished_since("33") is not None   # nothing consumed it yet
    ...
    rec = _patch_side_effects(monkeypatch)
    monkeypatch.setattr(daemon, "AUTOCF_RETRY_COOLDOWN", 900)
    _aged("33", 901, token="the-dead-run")                 # ...and once the cooldown passes
    armed = {"33": True}
    assert daemon._process_autocf({}, "33", {}, "%0", 95, "claude", set(), armed) is True
    assert rec["cf"] == [33]
...
def test_a_restart_after_a_successful_run_grants_no_retry(monkeypatch):
    ...
    assert daemon._process_autocf({}, "33", {}, "%0", 95, "claude", set(), armed) is False
    assert armed["33"] is True and rec["cf"] == []
...
def test_a_malformed_autocf_file_does_not_kill_the_warning_loop(monkeypatch):
    ...
    _aged("33", 901)
    monkeypatch.setattr(daemon, "AUTOCF_RETRY_COOLDOWN", 900)
    fired = _run_one_warning_poll(monkeypatch, {"33": {"pane": "%0"}})
...
def test_codex_is_still_never_fired_by_the_retry(monkeypatch):
    ...
    _aged("33", 901)
    armed = {"33": True}
    fired = daemon._process_autocf({}, "33", {}, "%0", 95, "codex", set(), armed)
    assert fired is False
    assert rec["cf"] == []
